...
export async function getCoordinates(place: string) {
  ...
  await axios.get(`https://maps.googleapis.com/maps/api/place/findplacefromtext/json?key=${apiKey}&input=${place}&inputtype=textquery&fields=geometry`).then(
    data => {
...
export async function getRestaurants(coords: Coords) {
  ...
  await axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?key=${apiKey}&location=${coords.lat},${coords.lng}&radius=${radius}&type=restaurant`).then(data => {
